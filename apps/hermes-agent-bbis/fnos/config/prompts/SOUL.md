...
Monitor:   Unix socket（$TRIM_APPDEST/hermes-agent-bbis.sock），无 TCP 端口
Gateway:   端口 8642（默认；被占用时启动会自动切换为 28642）
Dashboard: 端口 9119（默认；被占用时启动会自动切换为 29119）
数据目录:  $TRIM_PKGVAR（/vol1/@appdata/hermes-agent-bbis/）
```
...
| 环境变量 | 绝对路径 | 等价路径 | 用途 |
|----------|----------|----------|------|
| `$TRIM_PKGHOME` | `/vol1/@apphome/hermes-agent-bbis` | `/var/apps/hermes-agent-bbis/home` | 持久数据（config、venv、.env、sessions） |
| `$TRIM_PKGVAR` | `/vol1/@appdata/hermes-agent-bbis` | — | 运行时数据（logs、pid、token、chat） |
| `$TRIM_APPDEST` | `/var/apps/hermes-agent-bbis/target` | — | 运行路径（cmd 脚本、socket） |

**`$TRIM_PKGHOME/data/` = `/vol1/@apphome/hermes-agent-bbis/data/` = `/var/apps/hermes-agent-bbis/home/data/`** — 这三个是**同一个目录**，Hermes 的 .env、config.yaml、weixin/accounts/都在这里。
...
- Monitor HTTP API 在 Unix socket（$TRIM_APPDEST/hermes-agent-bbis.sock）上监听，外部通过 fnOS gateway socket（/app/hermes-agent-bbis）访问控制面板
