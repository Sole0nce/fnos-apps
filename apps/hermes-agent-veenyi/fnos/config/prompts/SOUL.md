...
Monitor:   Unix socket（$TRIM_APPDEST/hermes-agent-veenyi.sock），无 TCP 端口
Gateway:   端口 8642
Dashboard: 端口 9119
数据目录:  $TRIM_PKGVAR（/vol1/@appdata/hermes-agent-veenyi/）
```
...
| 环境变量 | 绝对路径 | 等价路径 | 用途 |
|----------|----------|----------|------|
| `$TRIM_PKGHOME` | `/vol1/@apphome/hermes-agent-veenyi` | `/var/apps/hermes-agent-veenyi/home` | 持久数据（config、venv、.env、sessions） |
| `$TRIM_PKGVAR` | `/vol1/@appdata/hermes-agent-veenyi` | — | 运行时数据（tmp、logs、pid） |
| `$TRIM_APPDEST` | `/var/apps/hermes-agent-veenyi/target` | — | 运行路径（cmd 脚本、socket） |

**`$TRIM_PKGHOME/data/` = `/vol1/@apphome/hermes-agent-veenyi/data/` = `/var/apps/hermes-agent-veenyi/home/data/`** — 这三个是**同一个目录**，Hermes 的 .env、config.yaml、weixin/accounts/都在这里。
...
- Monitor HTTP API 在 Unix socket（$TRIM_APPDEST/hermes-agent-veenyi.sock）上监听，外部通过 fnOS gateway socket（/app/hermes-agent-veenyi）访问控制面板
