...
> 🧭 **路径认知**：`/var/apps/hermes-agent-veenyi/home/data/` ≡ `/vol1/@apphome/hermes-agent-veenyi/data/`。优先用环境变量 `$TRIM_PKGHOME`（data 根）、`$TRIM_APPDEST`（程序目录），**勿硬编码挂载点**。若等价路径"看似为空"，用 `stat` 复核，不要横跳。
...
DATA_DIR = os.environ.get("TRIM_PKGHOME", "/var/apps/hermes-agent-veenyi/home") + "/data"
...
2. **后台启动**：`cd "${TRIM_PKGHOME:-/var/apps/hermes-agent-veenyi/home}/data" && nohup python3 /tmp/weixin_bind.py > /tmp/weixin_bind.log 2>&1 &`
...
```bash
DATA_DIR="${TRIM_PKGHOME:-/var/apps/hermes-agent-veenyi/home}/data"
...
8. **重启 Gateway**：
   ```bash
   curl -s -X POST --unix-socket "$TRIM_APPDEST/hermes-agent-veenyi.sock" \
...
```bash
SOCK="${TRIM_APPDEST:-/var/apps/hermes-agent-veenyi/target}/hermes-agent-veenyi.sock"
TOKEN=$(cat "${TRIM_PKGVAR:-/vol1/@appdata/hermes-agent-veenyi}/monitor.token")
...
| 变量 | 含义 | 典型值 |
|---|---|---|
| `$TRIM_PKGHOME` | data 根 | `/var/apps/hermes-agent-veenyi/home` |
| `$TRIM_PKGVAR` | 运行时数据 | `/vol1/@appdata/hermes-agent-veenyi` |
| `$TRIM_APPDEST` | 程序安装目录 | `/var/apps/hermes-agent-veenyi/target` |
