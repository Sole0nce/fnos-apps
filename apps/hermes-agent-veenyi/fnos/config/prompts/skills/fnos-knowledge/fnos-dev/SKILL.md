...
services:
  hermes-agent:
    ...
    ports:
      - "8642:8642"
    volumes:
      - /vol1/@appdata/hermes-agent-veenyi:/data
    environment:
      - TZ=Asia/Shanghai
```
...
```json
{
  ...
  "username": "hermes-agent-veenyi",
  "groupname": "hermes-agent-veenyi",
  "join-groups": []
}
```
...
APP_DIR="${TRIM_APPDEST:-/var/apps/hermes-agent-veenyi/target}"
DATA_DIR="${TRIM_PKGVAR:-/vol1/@appdata/hermes-agent-veenyi}"
...
```bash
#!/bin/bash
PID_FILE="${TRIM_APPDEST:-/var/apps/hermes-agent-veenyi/target}/run/hermes-agent.pid"
[ -f "$PID_FILE" ] || exit 0
...
**`$TRIM_PKGHOME/data`（`/vol1/@apphome/hermes-agent-veenyi/data/`）** — 应用核心数据：

```
/vol1/@apphome/hermes-agent-veenyi/data/
...
**`$TRIM_PKGVAR`（`/vol1/@appdata/hermes-agent-veenyi/`）** — 运行时数据：

```
/vol1/@appdata/hermes-agent-veenyi/
...
**Hermes Agent 建议**：`8642`（API Gateway）/ `9119`（Dashboard）/ Monitor 无 TCP 端口，仅通过 Unix socket（`$TRIM_APPDEST/hermes-agent-veenyi.sock`）通信
