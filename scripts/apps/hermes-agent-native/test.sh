...
${SUDO_SSH} "bash -c '
set -x
export TRIM_APPNAME=hermes-agent-native
...
sleep 2
test -S /tmp/hagent-test/app/run/hermes-agent-native.sock && echo \"socket created\"
/tmp/hagent-test/cmd/main status

echo \"=== 3. socket 502 fallback (cold start) ===\"
curl -s --unix-socket /tmp/hagent-test/app/run/hermes-agent-native.sock http://localhost/ -o /dev/null -w \"http=%{http_code}\\\\n\" --max-time 3 || echo \"(dashboard may be up already)\"

echo \"=== 4. browser flow ===\"
sleep 8
curl -s --unix-socket /tmp/hagent-test/app/run/hermes-agent-native.sock http://localhost/ -o /tmp/dash-index.html --max-time 10
...
curl -s --unix-socket /tmp/hagent-test/app/run/hermes-agent-native.sock http://localhost/api/status -H \"X-Hermes-Session-Token: \\${TOKEN}\" --max-time 10 | head -c 200; echo
