...
# Repackages the upstream fnos-hermes-agent fpk published by iranee/fnos-hermes-agent.
#
...
UPSTREAM_REPO="iranee/fnos-hermes-agent"

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"

echo "==> Repackaging Hermes Agent (bbis) ${VERSION} from upstream fpk"
...
WORK_DIR_MSYS="$REPO_ROOT/.tmp-build-hermes-agent-bbis"
...
if [ -f "$TMP_TGZ/ui/config" ]; then
    cp "$REPO_ROOT/apps/hermes-agent-bbis/fnos/ui/config" "$TMP_TGZ/ui/config"
...
# the app to hermes-agent-bbis the fnOS gateway prefix is
# /app/hermes-agent-bbis, so an unrewritten base href makes the whole UI
...
if [ -f "$TMP_TGZ/ui/index.html" ]; then
    sed -i 's|<base href="/app/hermes-agent/">|<base href="/app/hermes-agent-bbis/">|g' "$TMP_TGZ/ui/index.html"
    echo "==> Rewrote app.tgz ui/index.html base href -> /app/hermes-agent-bbis/"
else
