...
cp "${SCRIPT_DIR}/../../../apps/sub-store/fnos/docker/docker-compose.yaml" "${WORK_DIR}/docker/"
...
cp -a "${SCRIPT_DIR}/../../../apps/sub-store/fnos/ui" "${WORK_DIR}/ui"

cd "${WORK_DIR}"
tar czf "${SCRIPT_DIR}/../../../app.tgz" docker/ ui/

echo "Built app.tgz for sub-store ${VERSION}"
